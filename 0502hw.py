...
# month_generator 함수 만들기 
# text_object_dict : a dictionary used to store references to the text box objects created for each day in the calendar.
    ...
    for row in range(6):
        for column in range(7):
            if index >= start_date and index <= start_date + num_days - 1:
                ...
                day += 1
            index += 1
    # month_generator 함수에 load, save 기능 버튼 추가 및 함수 연결
...
# get_start_day_of_month 함수 만들기
def get_start_day_of_month(month, year):
    # 무슨 요일에 1일인지 찾는 함수
    last_two_year = year - 2000
    calculation = last_two_year // 4 + 6 # 2000-01-01이 토요일이어서 6 더해줌
    if month in [1, 10]:
        calculation += 1
...
def switch_months(direction):
    ...
    # 이미 존재하는 calendar_frame 이 있다면 destroy (다른 달로 넘어갔을 때 원래 내용 삭제하는거)
    if calendar_frame:
        calendar_frame.destroy()

    #calendar_frame '다시' 만들어주기 (재귀)
...
# main 함수 만들기 (메인 함수는 제일 밑에 가게)
month = date.today().month  # 위에서 선언한 date package 안에 있는
